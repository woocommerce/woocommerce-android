#!/usr/bin/env bash
# translate_naive.sh — translate Android strings using a naive (no context) LLM prompt
# Usage: ./translate_naive.sh <language_name> <language_code> <llm>
# Example: ./translate_naive.sh German de claude
...
PROMPT_TEMPLATE="$SCRIPT_DIR/prompts/naive.txt"
...
OUT_DIR="$SCRIPT_DIR/../results/translations/naive/$LLM"
mkdir -p "$OUT_DIR"
...
"$PYTHON" - "$ALL_STRINGS_FILE" "$CHUNKS_DIR" <<'PYEOF'
import json, sys, os

all_strings_file = sys.argv[1]
chunks_dir = sys.argv[2]
chunk_size = 200

with open(all_strings_file) as f:
    strings = json.load(f)
...
TOTAL_CHUNKS=$("$PYTHON" - "$ALL_STRINGS_FILE" <<'PYEOF'
...
for CHUNK_FILE in "$CHUNKS_DIR"/chunk_*.json; do
    ...
    PROMPT="${PROMPT/\{STRINGS_JSON\}/$STRINGS_JSON}"
