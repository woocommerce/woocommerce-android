...
| **Dashboard/Stats** | | | |
| Charts respond, date ranges | `dashboard_stats.yaml` | Yes | Asserts the `dashboard_stats_card`, matches any of the stat-tile labels (`Total sales`, `Revenue` fallback, `Orders`, `Visitors`) so string renames don't break it, opens the range dropdown and switches to "This Week" by tapping the menu-item text (the Compose `DropdownMenu` renders in a `Popup` whose `testTag` doesn't bubble into Maestro's tree — tapping the visible label avoids that), then asserts the `dashboard_top_performers_card` scrolls into view. Includes a safety back-press if the popup refuses to close |
...
| Product detail (universal rows, any product type) | `products_detail.yaml` | Partial | Opens the first product and asserts the rows rendered for EVERY product type — `Product type`, `Reviews`, `Categories`. `Price`, `Inventory`, `Shipping`, `Short description` are probed optionally because they're absent on booking / grouped / external / subscription products (the flow used to fail whenever the first product on staging was non-simple) |
| Product detail — Variations | `products_variations_and_tags.yaml` | Yes | Opens the product filters (`btn_product_filter`), picks Product type → Variable, applies, asserts the filtered list is non-empty (fails loudly with a clear seed-data message if no Variable products exist), opens the first Variable product, asserts the `Variations` + `Variations attributes` rows, taps Variations to load `variationList`, then opens the first variation and asserts `cardsRecyclerView` on the variation detail screen. Backs out cleanly without mutating anything |
| Product detail — Tags | - | No | The `Tags` ComplexProperty only renders when the product already has tags, and asserting it from the list-ordering side is fragile. Covered implicitly by `products_detail.yaml` as part of the optional scroll probe, not explicitly asserted |
...
- **`ensure_logged_in.yaml` self-heals.** If the device isn't signed in when a non-login flow starts (cold emulator, session wiped, token expired), the subflow detects the welcome screen and falls back to running `login.yaml` automatically. This means any flow can be invoked in isolation from a cold emulator — the operator doesn't need to run `login_successful.yaml` first as a prerequisite.
...
- **Compose `Popup` / `DropdownMenu` test tags don't bubble to Maestro's view tree.** Any selector relying on `testTag(...)` that's placed on a popup node will silently miss. Branch on the visible menu-item text instead (see `dashboard_stats.yaml`'s range-switch logic).
- **Maestro's `inputText` does NOT support Unicode** ([mobile-dev-inc/maestro#146](https://github.com/mobile-dev-inc/maestro/issues/146)). Use plain ASCII in any string you type into the app — no em-dashes, smart quotes, etc.
