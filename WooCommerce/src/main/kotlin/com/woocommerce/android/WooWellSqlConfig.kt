...
class WooWellSqlConfig(context: Context?) : WellSqlConfig(context, WellSqlConfig.ADDON_WOOCOMMERCE) {
    /**
     * Detect when the database is downgraded in debug builds so we can recreate all the tables and show a
     * toast alerting to the downgrade. The initial purpose of this was to avoid the hassle of devs switching
     * branches and having to clear storage and login again due to a version downgrade, but we've had a
     * couple cases where a beta build with a DB downgrade was released, resulting in a lot of crashes.
     */
