...
/**
 * Removes the push identity which older app versions stored in backed-up default preferences.
 *
 ...
 */
class LegacyPushPreferencesMigration(
    private val defaultPreferences: SharedPreferences
) : DataMigration<Preferences> {
    override suspend fun shouldMigrate(currentData: Preferences): Boolean =
        defaultPreferences.contains(LEGACY_UUID_KEY) || defaultPreferences.contains(LEGACY_TOKEN_KEY)

    override suspend fun migrate(currentData: Preferences): Preferences = currentData

    override suspend fun cleanUp() {
        check(defaultPreferences.edit().remove(LEGACY_UUID_KEY).remove(LEGACY_TOKEN_KEY).commit()) {
            "Failed to remove legacy push preferences"
        }
    }
