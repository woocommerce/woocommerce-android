...
class WooPosCartViewModel @Inject constructor(
    ...
    private fun trackCheckoutTapped(itemsInCart: Int, couponsInCart: Int) {
        viewModelScope.launch {
            analyticsTracker.track(
                CheckoutTapped.apply {
                    addProperties(mapOf("items_in_cart" to "$itemsInCart"))
                    addProperties(mapOf("coupons_in_cart" to "$couponsInCart"))
                }
            )
        }
    }
