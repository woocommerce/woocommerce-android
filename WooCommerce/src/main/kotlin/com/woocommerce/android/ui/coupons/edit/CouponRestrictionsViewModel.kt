...
class CouponRestrictionsViewModel @Inject constructor(
    ...
    fun onExcludedProductChanged(excludedItems: Set<ProductSelectorViewModel.ProductItemData>) {
        restrictionsDraft.update {
            val productIds = excludedItems.filter { it.remoteVariationId == null }.map { it.remoteProductId }
            val variationIds = excludedItems.filter { it.remoteVariationId != null }.mapNotNull { it.remoteVariationId }
            it.copy(excludedProductIds = productIds + variationIds)
        }
    }
