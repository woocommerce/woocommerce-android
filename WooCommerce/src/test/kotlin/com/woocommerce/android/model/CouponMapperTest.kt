...
class CouponMapperTest {
    ...
    @Test
    fun `when GMT expiry is malformed, then preserve local date without inferring GMT instant`() {
        val coupon = createCoupon(
            dateExpires = "2025-06-16T00:00:00",
            dateExpiresGmt = "invalid"
        ).toAppModel()
    ...
    @Test
    fun `when GMT expiry has invalid calendar date, then exact expiry is null`() {
        val coupon = createCoupon(
            dateExpires = "2025-03-01T00:00:00",
            dateExpiresGmt = "2025-02-30T13:45:30"
        ).toAppModel()

        assertThat(coupon.dateExpiresGmt).isNull()
        assertThat(coupon.dateExpiresLocal).isEqualTo(LocalDate.of(2025, 3, 1))
    }

    @Test
    fun `when GMT expiry has invalid hour, then exact expiry is null`() {
        val coupon = createCoupon(
            dateExpires = "2025-06-16T00:00:00",
            dateExpiresGmt = "2025-06-15T25:45:30"
        ).toAppModel()
    ...
    @Test
    fun `when GMT expiry has trailing garbage, then exact expiry is null`() {
        val coupon = createCoupon(
            dateExpires = "2025-06-16T00:00:00",
            dateExpiresGmt = "2025-06-15T13:45:30garbage"
        ).toAppModel()
