...
class AnalyticsRepositoryTest : BaseUnitTest() {
    ...
    @Test
    fun `given no previousPeriodRevenue then fetchRevenueData is null`() = runBlocking {
        // Given
    ...
    @Test
    fun `given no currentPeriodRevenue then fetchRevenueData is null`() = runBlocking {
        // Given
        val statsTotal = getStatsTotal()
        whenever(statsRepository.fetchRevenueStats(YEARS, true, PREVIOUS_DATE, PREVIOUS_DATE))
    ...
    @Test
    fun `given previousPeriodRevenue with no total sales then fetchRevenueData is the expected`() = runBlocking {
        // Given
        val statsTotal = getStatsTotal()
        ...
        // When
        val result: RevenueStat? = sut.fetchRevenueStatData(DateRange.SimpleDateRange(previousDate!!, currentDate!!),
            ANY_RANGE)
    ...
    private fun getStatsTotal(): WCRevenueStatsModel {
        val stats: WCRevenueStatsModel = mock()
        val revenueStatsTotal: WCRevenueStatsModel.Total = mock()
        whenever(revenueStatsTotal.netRevenue).thenReturn(TEN_VALUE)
        whenever(revenueStatsTotal.totalSales).thenReturn(TEN_VALUE)
