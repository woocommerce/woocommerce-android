...
class WooPosSearchByIdentifierRemoteTest {
    ...
    @Test
    fun `given normal product found, when search successful, then updates products cache`() = runTest {
        // GIVEN
        val identifier = "test-sku"
        val normalProduct = ProductTestUtils.generateWCProductModel(
            productId = 100L,
            productType = "simple"
        )
        ...
        sut.onProductsSearched(
            WCProductStore.OnProductsSearched(
                globalUniqueIdSearchQuery = identifier,
                searchQuery = null,
                searchResults = listOf(normalProduct),
                canLoadMore = false,
                isSkuSearch = WCProductStore.SkuSearchOptions.Disabled
            )
        )
        ...
        val productsCaptor = argumentCaptor<List<com.woocommerce.android.model.Product>>()
        verify(productsCache).addAll(productsCaptor.capture())

        assertEquals(normalProduct.remoteProductId, productsCaptor.firstValue.first().remoteId)

        sut.onCleanup()
    }

    @Test
    fun `given variation product found, when search successful, then updates products cache`() =
        runTest {
            // GIVEN
            val identifier = "variation-sku"
            val productId = 100L
            val variationId = 200L
            val variationProduct = ProductTestUtils.generateWCProductModel(
                productId = variationId,
                productType = "variation"
            ).copy(
                parentId = productId,
            ...
            sut.onProductsSearched(
                WCProductStore.OnProductsSearched(
                    globalUniqueIdSearchQuery = null,
                    searchQuery = identifier,
                    searchResults = listOf(variationProduct),
                    canLoadMore = false,
                    isSkuSearch = WCProductStore.SkuSearchOptions.Disabled
                )
            )
            ...
            assertEquals(variationProduct.remoteProductId, productsCaptor.firstValue.first().remoteId)
