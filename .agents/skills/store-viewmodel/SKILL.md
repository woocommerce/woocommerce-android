...
Create the ViewModel in tests by constructing `FragmentArgs.toSavedStateHandle()`:
...
class MyFeatureViewModelTest : BaseUnitTest() {
    ...
    private fun setup(itemId: Long = 1L) {
        viewModel = MyFeatureViewModel(
            savedStateHandle = MyFeatureFragmentArgs(itemId = itemId).toSavedStateHandle(),
    ...
    @Test
    fun `when screen loads, then data is fetched`() = testBlocking {
        whenever(repository.fetchData(any())).thenReturn(Result.success(listOf(item)))
        setup()
...
Use `getOrAwaitValue()` for `LiveData` and `runAndCaptureValues {}` to capture intermediate states or events. See the `store-tests` skill for full testing conventions.
