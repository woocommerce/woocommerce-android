...
| Cell | `WooCell`, `WooSettingsRow`, `WooSwitchSettingsRow` | Custom slot row; Material 3 `ListItem` remains only an internal comparison | production_initial | Light/dark, leading/trailing, switch row, disabled-ready API, long text, large font, RTL | `WooCell` is the public row primitive: title, optional description, optional leading/trailing slots, enabled state, and one row action. The layout follows the Cell and Cell Content evidence from Figma nodes `1186:13285`, `1181:12446`, and `1164:9190`: content-driven height with 24dp horizontal/vertical padding, 16dp slot/content gaps, and a 48dp minimum touch target rather than a fixed 90dp row. `WooSettingsRow` is a Store settings convenience wrapper over the same cell layout, not a separate compact density. `WooSwitchSettingsRow` keeps row-owned switch semantics. Accessibility contract: one row-level action only, decorative slots do not add duplicate semantics, and text wraps at large font scales. |
