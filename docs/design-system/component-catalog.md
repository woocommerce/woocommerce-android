...
| Icons | `WooIconButton`, `WooOutlinedIconButton`, `WooIconButtonEmphasis`; private catalog sample for icon catalog | Material 3 icon-button primitives with content-slot and ImageVector convenience overloads; outlined variant uses the Figma-backed 40dp shaped surface inside a 48dp target | production | Light/dark, neutral/primary emphasis, disabled-ready API, plain and outlined states, icon catalog preview-only | Production scope is icon-only actions, not icon naming policy. Plain `WooIconButton` has no border and uses the shared emphasis policy. Figma navigation-button node `1656:5112` maps to `WooOutlinedIconButton` with a 40dp large-radius outline and 18dp icon. Accessibility review result: non-blank content description is required and owned by button semantics; child icon semantics are decorative; 48dp Material touch target preserved. |
...
| Navigation Bar | `WooTopAppBar`, `WooTopAppBarAction`; private catalog sample for exploratory variants | Root-adaptive Material 3 wrapper with String/ImageVector convenience API, descriptor actions, and raw slot escape hatch | production | Light/dark, design-system root, legacy-compatible root, navigation/action states, icon and text descriptor actions, long title with two descriptor actions, long text action at 1.5x font, large font, RTL | Production scope is one top app bar tree whose chrome is selected by the root-provided internal top app bar appearance. Screens do not pass an appearance override. `DesignSystem` keeps the centered Figma navigation bar from node `1657:961`: `CenterAlignedTopAppBar`, centered `bodyLarge.strong` title, outlined nav/action icon buttons, primary text actions, surface background, and bottom divider. `LegacyCompatible` is temporary migration support for screens still under `WooThemeWithBackground`: it mirrors existing `Toolbar` chrome with `TopAppBar`, start-aligned title, plain nav/action icon buttons, Material text actions, `R.color.color_toolbar`, and no extra design-system divider. Retained XML/View screens that keep Activity toolbar behavior can use the PR7 XML toolbar overlay instead of `WooTopAppBar`; it reads the same promoted DS color/stroke/radius resources and keeps `setSupportActionBar(...)` plus View title/up/menu contracts intact. The first overlay pilot covers menu-less settings toolbar chrome only. Menu, overflow, `SearchView`, and collapsing toolbar screens need a later retained XML pilot before this overlay is trusted there, though the likely direction remains the same XML overlay rather than per-screen Compose chrome. `WooTopAppBarAction.Icon` intentionally has no emphasis parameter because the inspected Figma navigation button has one source-backed treatment; use standalone `WooIconButton` / `WooOutlinedIconButton` when a screen needs an explicitly emphasized icon outside the top bar. `WooTopAppBarAction.Text` covers action-side label buttons from the Figma top navigation source and caps long labels to the 136dp side-action budget before ellipsizing so the title area is not consumed. Raw action slots remain available only as an advanced escape hatch; raw content is rendered as supplied and is not appearance-adaptive. Accessibility review result: icon labels reject blank content descriptions, text actions reject blank labels, title remains visible text, 48dp icon targets are preserved, and the existing auto-mirror guard is preserved for navigation icons in RTL. |
