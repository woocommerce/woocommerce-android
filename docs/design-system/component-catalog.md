...
| Cell | `WooCell`, `WooSettingsRow`, `WooSwitchSettingsRow` | Custom slot row; Material 3 `ListItem` remains only an internal comparison | production_initial | Light/dark, leading/trailing, switch row, disabled-ready API, long text, large font, RTL | Generic Cell should be public because the contract can stay controlled: title, optional description, optional leading/trailing slots, enabled state, and one row action. Generic `WooCell` follows the recorded Cell density evidence from Figma nodes `1186:13285` / `1181:12446`: 90dp-ish minimum height, 24dp horizontal padding, and 20dp slot/content gaps. Settings rows remain compact production aliases/compositions for pilot needs. Accessibility contract: one row-level action only, decorative slots do not add duplicate semantics, generic cells preserve at least a 48dp touch target, and compact settings rows enforce 48dp minimum height with wrapping text. |
| Cell Content | `WooCellContent`; title/description content inside row APIs | Custom text composition inside rows | production_initial | Light/dark, title/description density, long text, large font | Public as the reusable text/content nucleus for generic and settings rows. It accepts an enabled flag so parent rows can apply the same source-backed disabled colors without adding separate click semantics. Generic and compact parent rows own density; content text wraps by default and does not expose an independent action. |
