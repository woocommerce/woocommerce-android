...
| --- | --- | --- | --- |
| `WooTopAppBar` | Design-system appearance: `WooTheme.colors.surface.default`, `surface.onDefault`, `primary`, `outlineVariant`; `WooTheme.text.bodyLarge.strong`, `labelLarge.emphasized`; `WooTheme.spacing.space1`; internal `WooStroke.extraThin` | production | An internal root-provided appearance selects chrome from the root foundation; screens do not pass an appearance override. `DesignSystem` wraps Material 3 `CenterAlignedTopAppBar` and matches Figma Top Navigation Bar node `1657:961` for centered title, 64dp bar height, surface background, outlined nav/action icon buttons, primary text label actions, and bottom divider. Navigation and icon action content use `surface.onDefault`; text label actions use `primary`; action spacing uses `space1` because icon actions already include touch-target padding. The 40dp navigation visual sits inside a 48dp target and text actions cap at the 136dp side-action budget; these values are not promoted as tokens. `LegacyCompatible` intentionally mirrors existing legacy toolbar chrome under `WooThemeWithBackground` with `R.color.color_toolbar`, start-aligned title, plain icon buttons, Material text actions, and no extra design-system divider; this compatibility branch is not a new design-system token decision. |
...
| `WooCell`, `WooCellContent`, `WooSettingsRow`, `WooSwitchSettingsRow` | `WooTheme.colors.surface.onDefault`, `surface.onVariant`, `surface.onLowest`; `WooTheme.text.titleMedium.emphasized`, `bodyMedium.regular`; `WooTheme.spacing.space1`, `space5`; `WooTheme.padding.padding4`, `padding7` | production_initial | Cells are content-driven with a 48dp minimum touch target, `padding7` horizontal row padding, `padding4` vertical row padding, and `space5` leading/content/trailing gaps. `WooCellContent` uses `space1` between title and description. Grouped cell containers should add `padding4` vertical content padding so container and row padding combine to the 24dp section-edge-to-content rhythm shown in multi-cell Figma examples. `WooSettingsRow` is a Store settings convenience wrapper over the same layout. Enabled title/decorative slots use `surface.onDefault`, descriptions use `surface.onVariant`, and disabled content uses `surface.onLowest` without a row-level alpha. |
