...
| `WooCell`, `WooCellContent`, `WooSettingsRow`, `WooSwitchSettingsRow` | `WooTheme.colors.surface.onDefault`, `surface.onVariant`, `surface.onLowest`; `WooTheme.text.titleMedium.emphasized`, `bodyMedium.regular`; `WooTheme.spacing.space1`, `space5`; `WooTheme.padding.padding7` | production_initial | Cells are content-driven with a 48dp minimum touch target, `padding7` horizontal/vertical padding, and `space5` leading/content/trailing gaps. `WooCellContent` uses `space1` between title and description. `WooSettingsRow` is a Store settings convenience wrapper over the same layout, not a separate compact density. Enabled title/decorative slots use `surface.onDefault`, descriptions use `surface.onVariant`, and disabled content uses `surface.onLowest` without a row-level alpha. |
