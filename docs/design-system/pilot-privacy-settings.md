...
- It has useful design-system coverage: page text, section headers, cells/rows, switches, icon buttons, dividers, progress indicator, scrolling, and snackbar-triggering behavior.
...
If a required component is not production-ready yet, either implement it first or document why the pilot temporarily uses legacy/current Compose.
...
Before treating this as the existing-Compose adoption template:
