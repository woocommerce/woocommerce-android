...
class WCCustomerStore @Inject constructor(
     ...
     */
    suspend fun getCustomersForSite(site: SiteModel) =
        customerDao.getCustomersForSite(site.localId())
    ...
    suspend fun getCustomerByRemoteId(site: SiteModel, remoteCustomerId: Long) =
        customerDao.getCustomerByRemoteId(site.localId(), remoteCustomerId)

    /**
     * returns a cached customers with provided remote ids or empty list if not in cache
     */
    suspend fun getCustomerByRemoteIds(site: SiteModel, remoteCustomerId: List<Long>) =
        customerDao.getCustomerByRemoteIds(site.localId(), remoteCustomerId)
    ...
    suspend fun deleteCustomersForSite(site: SiteModel) {
        customerDao.deleteCustomersForSite(site.localId())
    }
        ...
        return coroutineEngine.withDefaultContext(AppLog.T.API, this, "fetchCustomers") {
            ...
            when {
                ...
                response.result != null -> {
                    ...
                    if (offset == 0L) customerDao.deleteCustomersForSite(site.localId())
