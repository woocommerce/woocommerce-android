...
class AssistantSystemPromptProviderTest {
    ...
    fun `when prompt is built, then show cards is the only card producer including customers and analytics stats`() {
        ...
        assertThat(prompt).contains("analytics_orders")
        assertThat(prompt).contains("analytics_orders:after:")
        assertThat(prompt).contains("currency:none")
        assertThat(prompt).contains("analytics_revenue:after:")
