...
import org.wordpress.android.fluxc.store.WpComPushNotificationStore.DeviceUnregistrationError
import org.wordpress.android.fluxc.store.WpComPushNotificationStore.DeviceUnregistrationErrorType
...
class NotificationRestClient @Inject constructor(
        ...
        val request =
            WPComGsonRequest.buildGetRequest(
                ...
                { response: NotificationHashesApiResponse?, _ ->
                    ...
                    dispatcher.dispatch(
                        NotificationActionBuilder.newFetchedNotificationHashesAction(
                            payload
                        )
                    )
                },
    ...
    suspend fun disableNotificationsFor(notificationSetting: List<SiteNotificationSettingDto>): Response<Unit> {
        val url = WPCOMREST.me.notifications.settings.urlV1_1
        val body = mutableMapOf<String, Any>()
        body["blogs"] = notificationSetting
    ...
    data class SiteNotificationSettingDto(
        @SerializedName("blog_id") val siteId: Long,
        @SerializedName("devices") val devices: List<DevicesDto>,
    )

    data class DevicesDto(
        @SerializedName("device_id") val deviceId: String,
        @SerializedName("new_comment") val newComment: Boolean,
        @SerializedName("store_order") val storeOrder: Boolean,
    )
}
