...
class SiteStorePersistence @Inject constructor(
    ...
    suspend fun insertOrUpdateSite(site: SiteModel): Int {
        ...
        // Wrap the check-then-insert/update logic in a transaction to prevent TOCTOU (Time-Of-Check to Time-Of-Use)
        // race conditions when multiple FETCH_SITES run concurrently. Without this, another coroutine could insert the
        // same site between our "does it exist?" check and the subsequent insert, causing a UNIQUE constraint crash.
        return database.withTransaction {
            ...
            if (existingSite == null) {
                ...
                if (siteResult != null) {
                    ...
                    // We don't allow this --> DuplicateSiteException
                    ...
                    if (siteResult.origin == SiteModel.ORIGIN_WPCOM_REST &&
