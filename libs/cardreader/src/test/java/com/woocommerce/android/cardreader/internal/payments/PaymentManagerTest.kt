...
class PaymentManagerTest {
    ...
    @Test
    fun `when currency not supported, then error emitted`() = runTest {
    ...
    @Test
    fun `when currency supported, then flow initiated`() = runTest {
        whenever(paymentUtils.isSupportedCurrency(any(), any())).thenReturn(true)
    ...
    @Test
    fun `given Terminal not initialized, when flow started, then error emitted`() = runTest {
    ...
    @Test
    fun `when creating payment intent starts, then InitializingPayment is emitted`() = runTest {
        val result = manager.acceptPayment(createPaymentInfo())
            .takeUntil(InitializingPayment::class).toList()
    ...
    @Test
    fun `when creating payment intent fails, then error emitted`() = runTest {
    ...
    @Test
    fun `when creating payment intent fails, then mapTerminalError invoked`() = runTest {
        whenever(createPaymentAction.createPaymentIntent(anyOrNull()))
            .thenReturn(flow { emit(CreatePaymentStatus.Failure(mock())) })
    ...
    @Test
    fun `when collecting payment starts, then CollectingPayment is emitted`() = runTest {
        val result = manager.acceptPayment(createPaymentInfo())
    ...
    @Test
    fun `when collecting payment fails, then error is emitted`() = runTest {
        whenever(collectPaymentAction.collectPayment(anyOrNull()))
    ...
    @Test
    fun `when collecting payment intent fails, then mapTerminalError invoked`() = runTest {
        whenever(collectPaymentAction.collectPayment(anyOrNull()))
            .thenReturn(flow { emit(CollectPaymentStatus.Failure(mock())) })
    ...
    @Test
    fun `when processing payment starts, then ProcessingPayment is emitted`() = runTest {
        val result = manager.acceptPayment(createPaymentInfo())
    ...
    @Test
    fun `when processing payment fails, then error emitted`() = runTest {
        whenever(processPaymentAction.processPayment(anyOrNull()))
    ...
    @Test
    fun `when processing payment fails, then mapTerminalError invoked`() = runTest {
        whenever(processPaymentAction.processPayment(anyOrNull()))
            .thenReturn(flow { emit(ProcessPaymentStatus.Failure(mock())) })
    ...
    @Test
    fun `when receiptUrl is empty, then PaymentFailed emitted`() = runTest {
        whenever(processPaymentAction.processPayment(anyOrNull()))
            .thenReturn(
                flow {
                    emit(
    ...
    @Test
    fun `when receiptUrl is empty, then PaymentData for retry are empty`() = runTest {
        whenever(processPaymentAction.processPayment(anyOrNull()))
            .thenReturn(
                flow {
                    emit(
    ...
    @Test
    fun `when capturing payment starts, then CapturingPayment is emitted`() = runTest {
        val result = manager.acceptPayment(createPaymentInfo())
    ...
    @Test
    fun `when capturing payment succeeds, then PaymentCompleted is emitted`() = runTest {
        val result = manager.acceptPayment(createPaymentInfo())
    ...
    @Test
    fun `when capturing payment succeeds, then PaymentCompleted event contains receipt url`() = runTest {
    ...
    @Test
    fun `given payment already captured, when capturing payment, then PaymentCompleted is emitted`() = runTest {
        whenever(cardReaderStore.capturePaymentIntent(any(), anyString()))
    ...
    @Test
    fun `when capturing payment fails, then error emitted`() = runTest {
        whenever(cardReaderStore.capturePaymentIntent(any(), anyString()))
    ...
    @Test
    fun `when capturing payment fails, then mapCapturePaymentError invoked`() = runTest {
        whenever(cardReaderStore.capturePaymentIntent(any(), anyString()))
            .thenReturn(CapturePaymentResponse.Error.GenericError)
