...
class PaymentManagerTest {
    ...
    fun setUp() = runBlockingTest {
        ...
        whenever(createPaymentAction.createPaymentIntent(anyInt(), anyString(), anyString()))
            .thenReturn(flow {
    ...
    fun `when currency not USD, then error emitted`() = runBlockingTest {
        val result = manager.acceptPayment(DUMMY_ORDER_ID, DUMMY_AMOUNT, NONE_USD_CURRENCY, DUMMY_EMAIL).single()
    ...
    fun `when currency is USD, then flow initiated`() = runBlockingTest {
        val result = manager.acceptPayment(DUMMY_ORDER_ID, DUMMY_AMOUNT, USD_CURRENCY, DUMMY_EMAIL)
    ...
    fun `when payment flow started, then dollar amount converted to cents`() = runBlockingTest {
        ...
        manager.acceptPayment(DUMMY_ORDER_ID, BigDecimal(1), USD_CURRENCY, DUMMY_EMAIL).toList()

        verify(createPaymentAction).createPaymentIntent(captor.capture(), anyString(), anyString())
    ...
    fun `when amount $1 ¢005, then it gets rounded down to ¢100`() = runBlockingTest {
        ...
        manager.acceptPayment(DUMMY_ORDER_ID, BigDecimal(1.005), USD_CURRENCY, DUMMY_EMAIL).toList()
    ...
    fun `when amount $1 ¢006, then it gets rounded up to ¢101`() = runBlockingTest {
        ...
        manager.acceptPayment(DUMMY_ORDER_ID, BigDecimal(1.006), USD_CURRENCY, DUMMY_EMAIL).toList()
    ...
    fun `when amount $1 ¢99, then it gets converted to ¢199`() = runBlockingTest {
        ...
        manager.acceptPayment(DUMMY_ORDER_ID, BigDecimal(1.99), USD_CURRENCY, DUMMY_EMAIL).toList()
    ...
    fun `given Terminal not initialized, when flow started, then error emitted`() = runBlockingTest {
        ...
        val result = manager.acceptPayment(DUMMY_ORDER_ID, DUMMY_AMOUNT, USD_CURRENCY, DUMMY_EMAIL).single()
    ...
    fun `when creating payment intent fails, then error emitted`() = runBlockingTest {
        ...
        val result = manager.acceptPayment(DUMMY_ORDER_ID, DUMMY_AMOUNT, USD_CURRENCY, DUMMY_EMAIL).toList()
    ...
    fun `when creating payment intent fails, then mapTerminalError invoked`() = runBlockingTest {
        ...
        manager.acceptPayment(DUMMY_ORDER_ID, DUMMY_AMOUNT, USD_CURRENCY, DUMMY_EMAIL).toList()
